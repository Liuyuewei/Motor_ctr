...
{
	HAL_TIM_Base_Start_IT(&TIM4_Handler); 							//使能定时器 4 和定时器 4 更新中断
}
...
{
	TIM4_Handler.Instance			=	TIM4; 						//通用定时器 4
	TIM4_Handler.Init.Prescaler		=	psc;						//分频系数
	TIM4_Handler.Init.CounterMode	=	TIM_COUNTERMODE_UP; 		//向上计数器
	TIM4_Handler.Init.Period		=	arr; 						//自动装载值
	TIM4_Handler.Init.ClockDivision	=	TIM_CLOCKDIVISION_DIV1; 	//时钟分频因子
	
	HAL_TIM_Base_Init(&TIM4_Handler); 								//初始化定时器 4
//	HAL_TIM_Base_Start_IT(&TIM3_Handler); 							//使能定时器 4 和定时器 4 更新中断
}
