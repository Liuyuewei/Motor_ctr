...
{
    if(state == 1)
		rt_pin_write(eTem2_SDA,PIN_HIGH);
    else
		rt_pin_write(eTem2_SDA,PIN_LOW);
}
/*
* brief: IIC SET SCL.
* param: state  set 0 or 1
...
{
    if(state == 1)
       rt_pin_write(eTem2_SCL,PIN_HIGH);
    else
       rt_pin_write(eTem2_SCL,PIN_LOW);
}
/*
* brief: IIC GET SCL.
* param: None
...
{
    static struct rt_i2c_bus_device stm32_iic2;
    GPIO_Configuration();
    
    rt_memset((void *)&stm32_iic2,0,sizeof(struct rt_i2c_bus_device));
    stm32_iic2.ops = (void *)&stm32_i2c_bit_ops;
    stm32_iic2.priv = (void *)&stm32_i2c_bit_ops;
    return rt_i2c_bit_add_bus(&stm32_iic2,"iic2");
}
